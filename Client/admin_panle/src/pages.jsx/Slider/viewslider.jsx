      ...
      Swal.fire({
        ...
        .then((result) => {
          ...
          if (result.isConfirmed) {
            axios.post(`${apibaseurl}slider/change-status`, { _id: salectedids })
              .then((res) => res.data)
              .then((finalres) => {
    ...
    if (salectedids.length >= 1) {
      Swal.fire({
        ...
        .then((result) => {
          ...
          if (result.isConfirmed) {
            axios.post(`${apibaseurl}slider/delete`, { _id: salectedids })
              .then((res) => res.data)
              .then((finalres) => {
                toast.success(finalres._massage)
                Sliderdata()
  ...
  return (
    ...
    <ToastContainer/>
      <div className=' w-5xl mx-auto min-h-[88vh]'>
        <h2 className='ps-3 py-2'>Home/ViewSlider</h2>
        ...
        <div className='shadow p-2 w-5xl mx-auto bg-neutral-100'>
          ...
          <div className='shadow-xs shadow-violet-600 '> <h2 className='shadow-violet-900 text-2xl font-bold px-3 py-1 text-neutral-600'>Viewcatagory</h2>  </div>
...

                {
                  data.map((item, index) => (
                    <tr className="text-center">
                      ...
                      <td className="border p-2 font-serif font-medium"> {item._SliderTitle} </td>
                      <td className="border py-2">  {item._SliderOrder} </td>
                      ...
                      <td className="border p-2">  {item._SliderStatus? <button className='bg-green-500 px-2 py-1 rounded-xl text-white font-bold' > Active </button> : <button className='bg-orange-500 px-2 py-1 rounded-xl text-white' > Deactive </button>}      </td>
                      <td className="border p-2"> <button className='bg-red-500 px-2 py-1 rounded-xl text-white font-bold' onClick={() => Deletedata(item._id)} > Delate </button> | <Link to={`/slider/update_slider/${item._id}`} > <button className='bg-blue-500 font-bold px-2 py-1 rounded-xl text-white'> Edit </button></Link>  </td>
                    </tr>
