      ...
      Swal.fire({
        ...
        .then((result) => {
          ...
          if (result.isConfirmed) {
            axios.post(`${apibaseurl}category/change-status`, { _id: salectedids })
              .then((res) => res.data)
              .then((finalres) => {
    ...
    if (salectedids.length >= 1) {
      Swal.fire({
        ...
        .then((result) => {
          ...
          if (result.isConfirmed) {
            axios.post(`${apibaseurl}category/delete`, { _id: salectedids })
              .then((res) => res.data)
              .then((finalres) => {
                toast.success(finalres._massage)
                categorydata()
  ...
  return (
    ...
    <ToastContainer/>
      <div className=' w-5xl mx-auto min-h-[88vh]'>
        <h2 className='ps-3 py-2'>Home/Viewcatagory</h2>
...

                {
                  data.map((item, index) => (
                    <tr className="text-center">
                      ...
                      <td className="border p-2 font-serif font-medium"> {item._CategoryName} </td>
                      <td className="border py-2">  {item._CategoryOrder} </td>
                      ...
                      <td className="border p-2">  {item._CategoryStatus? <button className='bg-green-500 px-2 py-1 rounded-xl text-white font-bold' > Active </button> : <button className='bg-orange-500 px-2 py-1 rounded-xl text-white' > Deactive </button>}      </td>
                      <td className="border p-2"> <button className='bg-red-500 px-2 py-1 rounded-xl text-white font-bold' onClick={() => Deletedata(item._id)} > Delate </button> | <Link to={`/catagory/update_catagory/${item._id}`} > <button className='bg-blue-500 font-bold px-2 py-1 rounded-xl text-white'> Edit </button></Link>  </td>
                    </tr>
